...
import pinocchio.visualize as pinviz
...
class TsidBiped:
    ...
    def __init__(self, conf, viewer=pinviz.MeshcatVisualizer):
        self.conf = conf
        self.robot = tsid.RobotWrapper(
            str(conf.urdf), [str(conf.path)], pin.JointModelFreeFlyer(), False
        )
        robot = self.robot
        self.model = robot.model()
        pin.loadReferenceConfigurations(self.model, str(conf.srdf), False)
        ...
        if viewer:
            self.robot_display = pin.RobotWrapper.BuildFromURDF(
                str(conf.urdf), [str(conf.path)], pin.JointModelFreeFlyer()
            )
            if viewer == pinviz.GepettoVisualizer:
            ...
            elif viewer == pinviz.MeshcatVisualizer:
                self.viz = viewer(
                    self.robot_display.model,
                    self.robot_display.collision_model,
                    self.robot_display.visual_model,
