...
robot = tsid.RobotWrapper(str(conf.urdf), [str(conf.path)], False)
model = robot.model()
...
ncols = 2
nrows = int(robot.nv / ncols)

if PLOT_JOINT_POS:
    (f, ax) = plut.create_empty_figure(nrows, ncols)
    f.suptitle("Joint Position Tracking")
    f.canvas.manager.set_window_title("Joint Position Tracking")
...
if PLOT_JOINT_VEL:
    (f, ax) = plut.create_empty_figure(nrows, ncols)
    f.suptitle("Joint Velocity Tracking")
    f.canvas.manager.set_window_title("Joint Velocity Tracking")
    ax = ax.reshape(robot.nv)
    for i in range(robot.nv):
...
if PLOT_JOINT_ACC:
    (f, ax) = plut.create_empty_figure(nrows, ncols)
    f.suptitle("Joint Acceleration Tracking")
    f.canvas.manager.set_window_title("Joint Acceleration Tracking")
    ax = ax.reshape(robot.nv)
    for i in range(robot.nv):
