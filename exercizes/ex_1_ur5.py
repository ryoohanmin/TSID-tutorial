...
ncols = 2
nrows = int(tsid.robot.nv / ncols)

if PLOT_EE_POS:
    (f, ax) = plut.create_empty_figure(3, 1)
    f.suptitle("End-Effector Position Tracking")
    f.canvas.manager.set_window_title("End-Effector Position Tracking")
    for i in range(3):
        ax[i].plot(time, ee_pos[i, :], label="EE " + str(i))
        ax[i].plot(time, ee_pos_ref[i, :], "r:", label="EE Ref " + str(i))
        ax[i].set_title(f"Axis {'XYZ'[i]} Position")
        if i == 2:
...
if PLOT_EE_VEL:
    (f, ax) = plut.create_empty_figure(3, 1)
    f.suptitle("End-Effector Velocity Tracking")
    f.canvas.manager.set_window_title("End-Effector Velocity Tracking")
    for i in range(3):
        ax[i].plot(time, ee_vel[i, :], label="EE Vel " + str(i))
        ax[i].plot(time, ee_vel_ref[i, :], "r:", label="EE Vel Ref " + str(i))
        ax[i].set_title(f"Axis {'XYZ'[i]} Velocity")
        if i == 2:
            ax[i].set_xlabel("Time [s]")
...
if PLOT_EE_ACC:
    (f, ax) = plut.create_empty_figure(3, 1)
    f.suptitle("End-Effector Acceleration Tracking")
    f.canvas.manager.set_window_title("End-Effector Acceleration Tracking")
    for i in range(3):
        ...
        ax[i].set_title(f"Axis {'XYZ'[i]} Acceleration")
        if i == 2:
            ax[i].set_xlabel("Time [s]")
...
if PLOT_TORQUES:
    (f, ax) = plut.create_empty_figure(nrows, ncols)
    f.suptitle("Joint Torque")
    f.canvas.manager.set_window_title("Joint Torque")
...
if PLOT_JOINT_VEL:
    (f, ax) = plut.create_empty_figure(nrows, ncols)
    f.suptitle("Joint Velocity")
    f.canvas.manager.set_window_title("Joint Velocity")
    ax = ax.reshape(tsid.robot.nv)
    for i in range(tsid.robot.nv):
