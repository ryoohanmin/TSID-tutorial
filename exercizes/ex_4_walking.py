...
if PLOT_COM:
    ...
    f.suptitle("Center of Mass Position Tracking")
    if hasattr(f.canvas.manager, "set_window_title"):
        f.canvas.manager.set_window_title("Center of Mass Position Tracking")
    com_pos_titles = [
        "Axis X CoM Position",
        "Axis Y CoM Position",
        "Axis Z CoM Position",
    ...
    for i in range(3):
        ...
        ax[i].set_title(com_pos_titles[i])
    ...
    (f, ax) = plut.create_empty_figure(3, 1)
    f.suptitle("Center of Mass Velocity Tracking")
    if hasattr(f.canvas.manager, "set_window_title"):
        f.canvas.manager.set_window_title("Center of Mass Velocity Tracking")
    com_vel_titles = [
        "Axis X CoM Velocity",
        "Axis Y CoM Velocity",
        "Axis Z CoM Velocity",
    ...
    for i in range(3):
        ...
        ax[i].set_title(com_vel_titles[i])
    ...
    (f, ax) = plut.create_empty_figure(3, 1)
    f.suptitle("Center of Mass Acceleration Tracking")
    if hasattr(f.canvas.manager, "set_window_title"):
        f.canvas.manager.set_window_title("Center of Mass Acceleration Tracking")
    com_acc_titles = [
    ...
    for i in range(3):
        ...
        ax[i].set_title(com_acc_titles[i])
...
if PLOT_COP:
    ...
    f.suptitle("Center of Pressure Evolution")
    if hasattr(f.canvas.manager, "set_window_title"):
        f.canvas.manager.set_window_title("Center of Pressure Evolution")
    cop_titles = ["Axis X Center of Pressure", "Axis Y Center of Pressure"]
    for i in range(2):
...
if PLOT_FOOT_TRAJ:
    (f, ax) = plut.create_empty_figure(3, 1)
    f.suptitle("Foot Position Tracking")
    if hasattr(f.canvas.manager, "set_window_title"):
        f.canvas.manager.set_window_title("Foot Position Tracking")
    foot_titles = ["Axis X Foot Position", "Axis Y Foot Position", "Axis Z Foot Position"]
    for i in range(3):
        ax[i].plot(time, x_RF[i, :], label="Right Foot")
        ax[i].plot(time[:N], x_RF_ref[i, :], ":", label="Right Foot Ref")
        ax[i].plot(time, x_LF[i, :], label="Left Foot")
        ax[i].plot(time[:N], x_LF_ref[i, :], ":", label="Left Foot Ref")
        ax[i].set_title(foot_titles[i])
...
if PLOT_TORQUES:
    plt.figure("Normalized Joint Torque Usage")
    ...
    plt.plot([time[0], time[-1]], 2 * [1.0], ":")
    plt.gca().set_title("Normalized Joint Torque Usage")
...
if PLOT_JOINT_VEL:
    plt.figure("Normalized Joint Velocity Usage")
    for i in range(tsid_biped.robot.na):
        v_normalized = (
            2
    ...
    plt.plot([time[0], time[-1]], 2 * [-1.0], ":")
    plt.plot([time[0], time[-1]], 2 * [1.0], ":")
    plt.gca().set_title("Normalized Joint Velocity Usage")
    plt.gca().set_xlabel("Time [s]")
