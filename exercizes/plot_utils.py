...
def create_empty_figure(nRows=1, nCols=1, figsize=None, spinesPos=None, sharex=True):
    if figsize is None:
        if nCols == 1:
            figsize = (8.5, max(6.0, 2.6 * nRows + 1.5))
        else:
            figsize = (10.5, max(6.5, 2.4 * nRows + 1.5))

    f, ax = plt.subplots(
        nRows, nCols, figsize=figsize, sharex=sharex, constrained_layout=True
    )
    _position_figure_window(f, figsize)
    _register_quit_shortcuts(f)
...
def _register_quit_shortcuts(fig):
    def _on_key(event):
        if event.key in {"q", "ctrl+q", "cmd+q"}:
            plt.close("all")

    fig.canvas.mpl_connect("key_press_event", _on_key)


def _position_figure_window(fig, figsize):
    global _FIGURE_COUNTER

    try:
        manager = plt.get_current_fig_manager()
        window = manager.window
        screen = window.windowHandle().screen().availableGeometry()

        if screen.width() >= 1600:
            cols = 3
        elif screen.width() >= 900:
            cols = 2
        else:
            cols = 1

        col = _FIGURE_COUNTER % cols
        row = _FIGURE_COUNTER // cols

        target_w = min(int(figsize[0] * fig.get_dpi()), screen.width() // cols - 20)
        if row == 0:
            target_h = min(int(figsize[1] * fig.get_dpi()), screen.height() - 80)
        else:
            target_h = min(int(figsize[1] * fig.get_dpi()), screen.height() // 2 - 60)

        x = screen.x() + col * (screen.width() // cols) + 10
        y = screen.y() + (0 if row == 0 else screen.height() // 2) + 30

        window.setGeometry(x, y, max(target_w, 500), max(target_h, 400))
    except Exception:
        pass
...
mpl.rcParams["xtick.labelsize"] = DEFAULT_TICK_FONT_SIZE
mpl.rcParams["ytick.labelsize"] = DEFAULT_TICK_FONT_SIZE
